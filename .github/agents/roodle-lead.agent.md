...
### 2.5 送 QA 评审（质量控制，默认必走）
在派发之前，先调用 `runSubagent` 调用 `Roodle 质量控制` agent 对方案做事前评审：
- 把完整方案（包含契约 + 任务清单 + 顺序 + 风险）作为 prompt 传过去
- 拿到评审报告：
  - `APPROVE` → 继续派发
  - `APPROVE-WITH-COMMENTS` → 采纳建议后派发
  - `REJECT` / 有 BLOCKER → 修正方案后重新送评
- 如果需求极简单（例如只动一个字段、不跨端），可在告知用户后跳过
...
- 检查时间字段是否 Unix 秒
- 检查分页形态是否一致
- **抽样校验关键文件实际存在**（`list_dir` / `file_search`）——子 agent 可能虚构交付报告
- 不符就把 diff 和问题再派回去修

### 4.5 送 QA 验收（质量控制，默认必走）
实现完成后，在告知用户之前，调用 `Roodle 质量控制` agent 做独立验收：
- 传入：原始方案 + 实际改动文件清单（哪个子 agent 改了哪些文件 / 增加了哪些字段）
- 拿到验收报告：
  - `PASS` → 给用户出收敛报告
  - `PASS-WITH-FOLLOWUP` → 在收敛报告里列出遗留项
  - `FAIL` → 修复后重验，不要向用户报完成
